...
- SQL report import and query execution shall run outside the UI thread. A newer file load or query request shall supersede older pending presentation results so stale schema or table data cannot replace the current report state.
