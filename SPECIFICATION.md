...
- `.xvi` vector background references shall apply the XTherion placement offset in TH2 model coordinates and then render `XVIgrid`, `XVIshots`, `XVIsketchlines`, and station-table positions through the same TH2 scene projection as map geometry; they shall not be independently fitted like raster preview imagery. Station-table positions shall render as compact, zoom-stable markers without permanent labels. An `XVIshots` record with the XTherion LRUD passage vertices after its centreline endpoints shall render a filled, outlined passage envelope behind the shot line.
...
- While inserting a new `point station`, the map editor shall snap to a single nearby station from a visible `.xvi` background layer and write that station's exact `.xvi` name as `-name`. The snap shall use the layer's current placement and transform, shall not affect non-station points or existing objects, and shall be skipped when no station is near or more than one distinct station is eligible.
...
- Mapiah raster, `.xvi`, and SVG background transforms shall be composed the way Mapiah composes them: the layer shall be scaled from its `xx`/`yy` anchor, and the rotation shall turn around the layer pivot of the already scaled image. With no rotation the `xx`/`yy` anchor shall therefore be a fixed point of the transform.
- Changing a background layer's X or Y scale from the `Backgrounds` inspector shall keep the visible pivot marker at the same scene position, updating the anchor written to the layer metadata accordingly.
- Moving an `.xvi` layer that is backed by XTherion metadata and has no Mapiah-only transform shall update its XTherion `xth_me_image_insert` base position while preserving its root-station placement rather than upgrading it to Mapiah metadata.
