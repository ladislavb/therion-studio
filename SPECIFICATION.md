...
- Source-reference resolution for `input` and `source` shall treat leading `./` relative paths and Windows-style backslash separators as equivalent to normal forward-slash relative paths. The validator shall warn on path tokens that use backslash separators in source-reference commands and supported path options such as `export -output` / `-o`, and shall offer an explicit `Apply Fix` source edit that converts only the affected path token to portable `/` separators while previewing the full source line with the replacement applied.
