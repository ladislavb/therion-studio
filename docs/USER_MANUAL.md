...
- In a scrap with `-station-names <prefix> <suffix>`, station point names are validated after applying that prefix and suffix before the project checks the survey station reference. Use `[]` for an empty prefix or suffix.
...
Therion Studio also reads and writes Mapiah `##MAPIAH## image_insert_v1` background metadata for `format=xvi`, `format=raster`, and `format=svg` layers, including rotation, X/Y scale, and pivot. This allows rotated PocketTopo/XVI, raster, or SVG background references created in Mapiah to appear in the map editor, and lets Therion Studio upgrade a simple XTherion background reference to Mapiah metadata when you rotate, scale, or set the pivot for a layer. Moving an untransformed XTherion-backed XVI keeps its XTherion metadata. SVG references render as SVG layers using Mapiah intrinsic size and source viewBox metadata for placement. When you add an SVG background, Therion Studio derives that metadata from the SVG root `width`, `height`, and `viewBox` attributes. If a restored SVG is missing or invalid, the map status names that file while other valid background layers continue loading.

An `.xvi` background shows compact blue station markers at the station-table positions, as well as any LRUD passage envelopes. Passage envelopes appear as a subtle filled and outlined shape behind the centreline, so the background can be used directly as a drawing reference. Station markers remain a practical size while zooming and do not show permanent labels. The normal background-layer visibility and opacity controls apply to the complete `.xvi` reference.

When adding a station point in `Point` mode, click near one station on a visible `.xvi` background to place the point exactly at that station and fill its `-name`. If there is no nearby station, or more than one possible station, the point is inserted normally without automatic naming. The feature uses the current position, scale, and rotation of the background layer.
