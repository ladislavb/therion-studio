...
The SQL report viewer is read-only. Open a Therion `export database` `.sql` file from the `Files` pane to load it into an in-memory SQLite database. The query editor above the result table runs one read-only `SELECT` or `WITH` query. The right sidebar has `Presets` and `Schema` sections: presets fill the query editor and run predefined analyses for overview, survey lengths, exploration and surveying by person, yearly activity, recent activity, continuation stations, lead flags, entrances, depth by survey, and shot error ranking, while schema lists imported tables and columns. Mutating SQL statements are rejected, and very large result sets are capped in the viewer to keep the application responsive. `Export CSV` writes the currently displayed result table to a CSV file.
