...
- Keep the `2026.7.2` scope open for the focused performance sequence: Map runtime ownership R4-R5 and map refresh
  M0A immutable source-projection handles and M0B1 stale-selection generation guards are complete; establish M0B2
  generation-aware completion and M0C baseline measurements before widening only measured-safe partial refresh paths.
...
- Complete M0B2 scene-generation completion publication and M0C map-refresh measurements before widening partial refresh.
