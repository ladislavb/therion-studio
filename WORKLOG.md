...
- Keep Windows installer deployment passing `--qmldir resources/qml` to `windeployqt` and smoke-checking deployed
  `qml/QtQuick/Controls`, `Layouts`, and `Templates` in the install prefix used by `qt.conf` so the 3D viewer inspector
  does not install as a blank Quick surface.
...
- Keep logical command and token lookup available by absolute source offset so future editor projections can share DOM
  cursor mapping instead of rebuilding line/column scans per mode.
