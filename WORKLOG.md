...
- Keep project-template service coverage as a named QTest slot in `MainWindowServiceQTests` alongside the other
  application-service workflows.
- Continue migrating touched hand-rolled tests to QTest where the dependency/runtime boundary is already clear; core background-metadata, command-option-parser, map-background-placement, document-parser, and XVI-parser coverage now runs as named slots in `TherionCoreQTests`.
